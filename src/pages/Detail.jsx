...
export default function DetailPage() {
    ...
    fetchDetail(detailPath)
      .then(res => {
        if (res.success && res.data) {
          setData(res.data);
          // Auto-play trailer after 4 seconds
  ...
  async function playVideo(epIdx = -1, sIdx = 0) {
    ...
    try {
      ...
      else if (downloads.length > 0) { finalUrl = downloads[0].url; }
  ...
  return (
    <div className="detail-page">
      ...
      <div className="detail-hero">
        ...
        <div style={{
          position:'absolute', bottom: 50, left: 48, maxWidth: 550, zIndex: 5,
        }}>
          <h1 className="detail-title" style={{ marginBottom: 12 }}>{data.title}</h1>
          ...
          <div style={{ display:'flex', gap:12, alignItems:'center' }}>
            <button className="btn-watch" onClick={handleWatchBtn} disabled={playerLoading}
              style={{ opacity:playerLoading?0.7:1, minWidth: 220 }}>
              {playerLoading
                ? <><div className="spinner" style={{ width:18,height:18,borderWidth:2 }} /> Memuat...</>
                : <><i className="fas fa-play" /> {watchLabel}</>}
            </button>
            {data.trailerUrl && (
              <button className="btn-watch" style={{ background:'rgba(255,255,255,0.1)', border:'1px solid rgba(255,255,255,0.2)', color:'#ccc', minWidth:'auto', padding:'14px 20px' }}
      ...
      {/* ── CONTENT BELOW HERO ── */}
      <div className="detail-content" style={{ marginTop: 30 }}>
          ...
          <section style={{ marginBottom: 30 }}>
            ...
            <div style={{
              ...
              {episodes.map((ep,ei)=>{
                ...
                return (
                  <div key={ei}
                    onClick={()=>playVideo(ei,currentSeason)}
                    style={{
                  ...
                  >
                    ...
                    <div style={{
                      ...
                      {ep.thumbnail ? (
                        ...
                        <i className="fas fa-play" style={{ fontSize: 24, color: '#333' }} />
                      )}
                      {/* Prime-style episode number overlay */}
                      <div style={{
                    ...
                    <div style={{ padding: '12px 14px' }}>
                      <div style={{
                        fontSize: 14, fontWeight: 700, color: isActive ? '#fff' : '#ddd',
